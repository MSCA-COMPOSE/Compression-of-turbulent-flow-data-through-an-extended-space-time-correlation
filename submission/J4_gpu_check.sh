#!/bin/bash
# Stage 4 - Reconstruct a set of selected snapshots as Phi . V^T + mean and
#           compare them against the original snapshots, reporting the relative
#           reconstruction error per snapshot.
...
python ../hpda/4_check_RED.py ./$SLURM_JOB_ID-scheduler.json $INPUTDIR $SEQFILE $OUTDIR $SNAPS $TARGET
