...
if __name__ == '__main__':
    ...
    u = da.concatenate(Us, axis=-1)
    v = da.concatenate(Vs, axis=-1)
    w = da.concatenate(Ws, axis=-1)
    u = u.reshape(nxp*nyp,nzp*snaps).rechunk((nx_chunk1*ny_chunk1,nzp*snaps))
    v = v.reshape(nxp*nyp,nzp*snaps).rechunk((nx_chunk1*ny_chunk1,nzp*snaps))
    w = w.reshape(nxp*nyp,nzp*snaps).rechunk((nx_chunk1*ny_chunk1,nzp*snaps))
    ...
    u=u.rechunk((nx_chunk*ny_chunk,nzp*snaps))
    v=v.rechunk((nx_chunk*ny_chunk,nzp*snaps))
    w=w.rechunk((nx_chunk*ny_chunk,nzp*snaps))
    u,v,w = dask.persist(u,v,w)
    wait([u,v,w])
    EV_c=load_V_as_dask(args.output_dir)
    EV_c=EV_c.rechunk((-1,EV_c.shape[1]//7))
    EV_c, =dask.persist(EV_c)
    wait([EV_c])
    metadata = {
        "Phi_row_chunks": list(u.chunks[0]),
        "Phi_col_chunks": list(EV_c.chunks[1]),
        "dtype": "float32",
        "shape": EV_c.shape,
        "nx_chunk": nx_chunk,
        "ny_chunk": ny_chunk,
    ...
    Compute_POD_redV(u, EV_c, args.output_dir, "u")
    Compute_POD_redV(v, EV_c, args.output_dir, "v")
    Compute_POD_redV(w, EV_c, args.output_dir, "w")    
    # write_C_to_h5(C, args.output_dir, n_files=num_fil, row_chunk=C_chunks)
