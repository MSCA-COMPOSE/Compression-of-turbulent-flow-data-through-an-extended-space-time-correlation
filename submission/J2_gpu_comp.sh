#!/bin/bash
# Stage 2 - Randomized GPU eigendecomposition of C, selection of the number of
#           modes k for the target energy, and writing of the reduced eigenvectors V.
# The number of nodes is passed by run_comp_pipeline.sh (production used ~2).
...
# For the eigendecomposition a fractional memory limit (0.25) is typically enough.
srun dask-worker --interface ib0 --nthreads 8 --memory-limit 0.25 \
...
python ../hpda/2_Comp_Eig.py ./$SLURM_JOB_ID-scheduler.json $INPUTDIR $SEQFILE $OUTDIR $TARGET
